...
export function Input({
  ...
  const [value, setValue] = useState<string | string[]>(
    multi ? undefined : getDefaultValue(props)
  );
  ...
  useEffect(() => {
    if (props.value !== undefined) {
      ...
      if (multi) {
        setMultiValue(
          props.value?.map
            ? props.value.map((x) => ({ label: x, value: x }))
            : []
        );
      }
    } else {
  ...
  const onKeyDown = async (event) => {
    ...
    switch (event.key) {
      ...
      case "Tab":
        ...
        setMultiValue(
          multiValue.concat({ label: multiInputValue, value: multiInputValue })
        );
        setMultiInputValue("");
    }
  };
  ...
  return !multi ? (
      ...
      {number ? (
        <MaskedInput
          ...
          className={`bg-transparent self-stretch flex-grow px-3 ${
            ...
            props.suffix || props.clearable || type == "password"
              ? "pr-1.5"
              : ""
          } ${inputClassName || ""}`}
          id={props.id}
    ...
    <CreatableSelect
      ...
      id={props.id}
      name={props.name}
      className={`${controlClassName} px-0 ${
        props.error ? "error" : ""
      } ${className}`}
      style={{ ...style }}
