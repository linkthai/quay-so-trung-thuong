...
export function Checkbox({
  ...
  useEffect(() => {
    if (
      multi &&
      (props.cols || (options?.length && options.find((x) => x.cols)))
    ) {
      setIsGrid(true);
    } else {
      setIsGrid(false);
  ...
  const onClickOption = (val) => {
    ...
    if (index >= 0) {
      ...
      newValues = options
        .filter(
          (option) =>
            (value as string[]).includes(option.value) || option.value == val
        )
        .map((x) => x.value);
      setValue(newValues);
    }
  ...
  return (
    <div
      className={`pl-1 space-x-1 ${
        isGrid ? "grid grid-cols-12" : "flex"
      } ${className}`}
    >
      {!multi && (
        <div
        ...
        options?.map((option) => (
          <div
            ...
            className={`${controlClassName} ${
              props.cols || option.cols
                ? `col-span-${option.cols || props.cols}`
                : ""
            } ${props.error ? "error" : ""} ${
              props.readonly ? "readonly" : ""
            } ${
